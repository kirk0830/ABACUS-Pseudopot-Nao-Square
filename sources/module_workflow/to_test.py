...
import module_software.abacus.generation as abacus
import module_software.qespresso.generation as qe
import os
import module_io.output_pw as op
import module_io.output_lcao as ol
...
def _abacus_(test_status: dict, basis_type: str):
    ...
    for system in test_status["tests"].keys():
        ...
        if basis_type == "lcao":
            ...
            _stru, _cell = abacus.STRU_cif(fname=_cif_name,
...
def _qespresso_(test_status):
    ...
    for system in test_status["tests"].keys():
        _cif_name = "mp-" + system.split("_")[-1] + ".cif"
        fname = qe.cif_to_qespresso(cif_file=_cif_name)
        os.rename(fname, "qespresso_"+system+".in")
